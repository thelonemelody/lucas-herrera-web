...
export function CalendarDay({ day, post, isCurrentMonth = true }: CalendarDayProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-48 md:w-64 p-3 bg-space-dark border border-amber-glow/50 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none">
        ...
        <div className="flex flex-wrap gap-1">
          {post.tags.map((tag) => (
            <span
              key={tag}
              className="text-[9px] md:text-[10px] px-1.5 py-0.5 bg-amber-glow/20 text-amber-glow rounded"
            >
              {tag}
            </span>
          ))}
        ...
        {/* Tooltip arrow */}
        <div className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-l-8 border-r-8 border-t-8 border-transparent border-t-amber-glow/50" />
      </div>
    </Link>
  );
}
